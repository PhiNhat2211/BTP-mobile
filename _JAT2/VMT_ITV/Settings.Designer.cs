...
namespace VMT_ITV
{
    public class CustomSettingsProvider : SettingsProvider
    {
        ...
        private void SaveValuesToFile()
        {
            ...
            XElement settingsSection = import.Element(CONFIG).Element(USER_SETTINGS).Element(typeof(Properties.Settings).FullName);

            //iterate though the dictionary, either updating the value or adding the new setting.
            foreach (var entry in SettingsDictionary)
            {
                XElement setting = null;
                foreach (XElement element in settingsSection.Elements())
                {
                    if (element.Attribute(NAME).Value == entry.Key)
                    {
                        setting = element;
                        break;
                    }
            ...
            XNode node = settingsSection.FirstNode;
            while (node != null)
            {
                if (node is XElement)
                {
                    String strValue = (node as XElement).Value;
                    if (strValue == String.Empty ||
                        strValue.Equals(""))
                    {
                        XElement delXElement = (node as XElement);
                        node = node.NextNode;
                        delXElement.Remove();
                    }
                    else
        ...
        private string UserConfigPath
        {
            get
            {
                return AppSettings.Local.SettingsKey;
            }
        }
    ...
    [global::System.Configuration.SettingsProvider(typeof(CustomSettingsProvider))]
    internal sealed partial class AppSettings : global::System.Configuration.ApplicationSettingsBase
    {
        private static AppSettings localInstance = ((AppSettings)(global::System.Configuration.ApplicationSettingsBase.Synchronized(new AppSettings())));
