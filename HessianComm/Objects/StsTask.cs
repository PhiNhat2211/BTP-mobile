using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;

namespace HessianComm.Objects
{
    public class StsTask
    {
        public Machine workingMchn { get; set; }

        public Machine partnerMchn { get; set; }

        public Container cntr { get; set; }

        public Location loc { get; set; }

        public string jobTp { get; set; }

        public string twinTandemCd { get; set; }

        public string twinTandemKey { get; set; }

        public string positionOnChassis { get; set; }

        public string neighborYt { get; set; }

        public string jobSts { get; set; }

        public Vessel vsl { get; set; }

        public string taskId { get; set; }
    }
}
